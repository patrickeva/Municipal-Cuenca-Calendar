...
import { LayoutDashboard, CalendarDays, Tag, Settings, LogOut, Eye, Shield, X } from 'lucide-react';
...
export default function AdminSidebar({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`w-72 flex flex-col h-screen overflow-hidden flex-shrink-0
          fixed inset-y-0 left-0 z-50 transition-transform duration-300 ease-in-out
          lg:static lg:translate-x-0
          ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{ background: 'linear-gradient(180deg, #0c1f3f 0%, #0a1a35 100%)' }}
      >
        ...
        <div className="px-4 pb-6 pt-3">
          ...
          <button
            ...
            className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-sm font-semibold text-white/35 hover:bg-red-500/15 hover:text-red-300 transition-all duration-200 mt-1"
          >
            <span className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-white/5">
              <LogOut size={16} />
            </span>
            Sign Out
          </button>
